...
document.addEventListener("DOMContentLoaded",()=>{
    ...
    btn_login?.addEventListener("click",()=>{
        ...
        // !txt_username : txt_username 값이 "" 이면 
        // (username 에 값을 입력하지 않고 로그인 버튼을 누른다면)
        if(!txt_username) {
            alert("USER NAME 을 입력해 주세요");
            input_username.focus();
            // js 에서는 함수 코드의 진행을 중단할 때 return false 실행
            return false;
        }
        // !txt_password : txt_password 값이 "" 이면 
        if(!txt_password) {
