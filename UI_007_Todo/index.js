...
비로소 JS 를 실행하기 시작한다.
JS 코드가 html 위, 아래 어디에 있던지 상관없이 실행 가능하다.
*/
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    요청을 하면 화면에서 최초로 발견된 요소만 select 가 된다
    */
    ulContent.addEventListener("click", (event) => {
        ...
        */
        ...
        if (target.tagName === "SPAN") {
            ...
            /*
                target.closest("LI")
                클릭된 요소(target)를 감싸고 있는 부모들 중에
                가장 가까이 있는 LI tag.
             */
            const liTag = target.closest("LI");
            // LI tag 에 complete 라는 class 를 추가하라.
            // <li class="complete"> 와 같은 코드가 된다.
            // liTag.classList.add("complete");

            /*
            tag 요소에 class 속성이 설정되면 classlist 에 해당 클래스명이 배열로 저장되어 있다.
            즉, liTag(LI) 에 class 속성이 있고, 값이 complete 이면 제거
            만약 없으면 추가

            클래스변수?.aaa : 클래스변수가 null 이 아닌 경우에만 aaa 코드 실행.
    ...
    const closeBtn = document.querySelector("SIU");
